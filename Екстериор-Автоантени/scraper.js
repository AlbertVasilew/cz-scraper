...
axios.get(`${siteDir}/%D0%9D%D0%B0%D1%87%D0%B0%D0%BB%D0%BE/1/2/51/null/`,{
        ...
        if(isAvailable) {
            ...
            const additionalData = await axios.post(`${siteDir}/Functions/Requests/descirption.php`, `id=${id}`).then(response => {
                ...
                let description  = additionalData(".right")
                    .contents()
                    .toArray()
                    .filter(x => x.name == 'ul' || x.name == 'ol' || x.name == 'span' || x.name == 'p' || x.name == 'h4')
            ...
            return {
                ...
                "Промоционална цена:": "",
                "Редовна цена:": price,
                "Категории": "Екстериор, Екстериор > Автоантени",
                "Етикети": "",
                "Клас на доставка": "",
                ...
                "Мета: rank_math_primary_product_cat": '845',
